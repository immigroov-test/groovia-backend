...
-- Force the general commission to 10% on a re-run (the INSERT above is DO NOTHING). Per-mentor
-- overrides still win. Testing only; production respects whatever the developer set.
UPDATE platform_settings SET value = '10' WHERE key = 'immigroov_markup_pct';
...
-- Flag every imported mentor that still has no per-hour rate (Yokesh + the migrated roster) so the
-- first-login flow fires for them. hourly_rate IS NULL guards it: a mentor who already set a rate
...
INSERT INTO platform_settings (key, value, description) VALUES
  ('fx_max_age_minutes', '43200', 'Testing: 30 days so seeded FX never goes stale (no dispatcher). Prod: ~24h.')
