...
BEGIN
  ...
  IF m_id IS NULL THEN
    ...
    VALUES ('yokesh-dhanabal', 'Yokesh Dhanabal', 'dhanabalyokesh99@gmail.com', 'seed-yokesh-dhanabal',
            TRUE, 'approved', 'IN', 'IN', ARRAY['NL','IN']::CHAR(2)[], ARRAY['job_career'],
            ARRAY['en','ta'],
            ...
            'INR', TRUE, 'yokesh-dhanabal', 'Asia/Kolkata')
  ...
  ELSE
    UPDATE mentors SET
      ...
      country = 'IN', home_country_code = 'IN', expertise_country_codes = ARRAY['NL','IN']::CHAR(2)[],
  ...
  IF NOT EXISTS (SELECT 1 FROM weekly_availability WHERE mentor_id = m_id) THEN
    INSERT INTO weekly_availability (mentor_id, weekday, start_time, end_time, timezone, is_active)
    SELECT m_id, d, TIME '10:00', TIME '17:00', 'Asia/Kolkata', TRUE
...
-- Clear the random pravatar placeholder that the (now-disabled) seed backfill used to stamp onto
-- every photo-less mentor - including migrated ones, since they match profile_id IS NULL. A NULL
-- photo_url makes the UI fall back to the mentor's initials avatar (what the user actually wants).
UPDATE mentors SET photo_url = NULL WHERE photo_url LIKE '%pravatar%';
...
-- Testing uses a long FX max-age (30 days) so the seeded static FX never goes stale between setup
-- runs (no FX dispatcher needed in test). Otherwise get_fx_or_null returns NULL after 24h and every
-- price falls back to the mentor's own currency (the "still showing INR" bug). Production keeps ~24h.
INSERT INTO platform_settings (key, value, description) VALUES
  ('fx_max_age_minutes', '2880', 'Max age (min) of an FX rate before bookings fail with FX_UNAVAILABLE. 48h = use the latest daily rate, and if a refresh fails still accept yesterday''s (last-day fallback) instead of blocking checkout. refresh_fx_rates() runs on app startup (every cold start) and should also run daily (scripts/refresh_fx_rates.py).')
...
-- These are a REAL EUR-pivot snapshot (quote units per 1 EUR), NOT round placeholders: a placeholder
-- like EUR->INR=90 (real ~110) silently mis-priced every INR->EUR booking by ~22%. This snapshot is a
-- bootstrap only - the backend runs refresh_fx_rates() on startup and daily (scripts/refresh_fx_rates.py),
-- which overwrites all of these with the latest live rates, so the seed is superseded within moments of
-- the app coming up. Keep it roughly current when you regenerate the DB. DO UPDATE re-stamps fetched_at.
INSERT INTO fx_rates (base, quote, rate, as_of, fetched_at) VALUES
  ('EUR','USD',1.1548,CURRENT_DATE,NOW()), ('EUR','GBP',0.8569,CURRENT_DATE,NOW()),
  ('EUR','INR',109.95,CURRENT_DATE,NOW()), ('EUR','AUD',1.6373,CURRENT_DATE,NOW()),
  ('EUR','CAD',1.6122,CURRENT_DATE,NOW()), ('EUR','SGD',1.4779,CURRENT_DATE,NOW()),
  ('EUR','AED',4.2411,CURRENT_DATE,NOW()), ('EUR','JPY',182.4,CURRENT_DATE,NOW()),
  ('EUR','CHF',0.9344,CURRENT_DATE,NOW()), ('EUR','CNY',7.8019,CURRENT_DATE,NOW()),
  ('EUR','ZAR',18.6848,CURRENT_DATE,NOW()), ('EUR','BRL',5.8909,CURRENT_DATE,NOW()),
  ('EUR','NZD',1.9622,CURRENT_DATE,NOW()), ('EUR','SEK',10.9516,CURRENT_DATE,NOW()),
  ('EUR','NOK',10.9826,CURRENT_DATE,NOW()), ('EUR','DKK',7.4617,CURRENT_DATE,NOW()),
  ('EUR','PLN',4.2989,CURRENT_DATE,NOW()), ('EUR','HKD',9.0564,CURRENT_DATE,NOW()),
  ('EUR','MXN',19.8185,CURRENT_DATE,NOW()), ('EUR','THB',38.1132,CURRENT_DATE,NOW()),
  ('EUR','MYR',4.7222,CURRENT_DATE,NOW()), ('EUR','IDR',20635.21,CURRENT_DATE,NOW()),
  ('EUR','PHP',70.2173,CURRENT_DATE,NOW()), ('EUR','VND',30068.27,CURRENT_DATE,NOW()),
  ('EUR','KRW',1633.53,CURRENT_DATE,NOW()), ('EUR','TRY',55.1236,CURRENT_DATE,NOW()),
  ('EUR','SAR',4.3306,CURRENT_DATE,NOW()), ('EUR','BDT',143.05,CURRENT_DATE,NOW()),
  ('EUR','PKR',320.85,CURRENT_DATE,NOW()), ('EUR','LKR',387.12,CURRENT_DATE,NOW()),
  ('EUR','NPR',175.91,CURRENT_DATE,NOW()), ('EUR','NGN',1573.53,CURRENT_DATE,NOW()),
  ('EUR','KES',149.5,CURRENT_DATE,NOW()), ('EUR','EGP',57.4449,CURRENT_DATE,NOW()),
  ('EUR','TWD',37.2763,CURRENT_DATE,NOW()), ('EUR','ILS',3.4669,CURRENT_DATE,NOW()),
  ('EUR','RON',5.2532,CURRENT_DATE,NOW()), ('EUR','CZK',24.2611,CURRENT_DATE,NOW()),
  ('EUR','HUF',363.73,CURRENT_DATE,NOW())
...
-- BUG-139: re-declare my_bookings (defined above, before the reviews table existed) with a
-- `reviewed` flag, so the mentee dashboard can show "Leave a review" vs "Edit your review"
-- for completed sessions without an extra per-row API call.
DROP FUNCTION IF EXISTS my_bookings(UUID);
CREATE OR REPLACE FUNCTION my_bookings(p_candidate_id UUID)
RETURNS TABLE (
  id               UUID,
  ...
  pay_currency     TEXT,
  pay_state        TEXT,
  reviewed         BOOLEAN        -- true once the mentee has left a review for this booking
) LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT
    b.id, b.status::TEXT, b.slot_time, b.slot_end, b.meeting_url,
    s.title, s.duration,
...
-- ============================================================================
-- Legal Documents CMS  (folded in from legal_documents_setup.sql)
-- ----------------------------------------------------------------------------
-- Versioned, role-targeted legal documents: the admin CMS, an immutable
-- publication history, and per-user acknowledgement tracking. This is the SAME
-- schema as migrations/legal_documents_setup.sql, which stays the authored source
-- and is what you run against an EXISTING database. Keep the two in sync.
--
-- Placed here because it depends on profiles, mentors, user_role and
-- log_audit_event(), all created above.
--
-- Document TEXT is NOT seeded here - only the 14 catalogue rows. Load the content
-- with:  python -m scripts.seed_legal_documents
-- ============================================================================
...
-- ============================================================================
-- Active-consent capture  (folded in from legal_consent_flow_setup.sql)
-- ----------------------------------------------------------------------------
-- Central log for every ACTIVE consent event (signup, guest checkout, mentor
-- onboarding, the Groovia AI Terms gate, the cookie banner) plus the Data
-- Subject Rights intake ticket. This is the SAME schema as
-- migrations/legal_consent_flow_setup.sql, which stays the authored source and
-- is what you run against an EXISTING database. Keep the two in sync.
--
-- Placed here (after the Legal Documents CMS block) because it references
-- legal_documents / legal_document_versions, both created above.
-- ============================================================================
